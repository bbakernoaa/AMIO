...
# `amio_discovery_tensorstore_absent_emits_fallback_warning` CTest
# entry.
#
# Verifies (Mermaid path: A -> B[FORCE_NCZARR?] -- OFF -->
#                          C[find_package(TensorStore CONFIG)] ->
#                          D[FOUND?] -- NO --> F (fallback warning)):
#
#   1. AMIO_HAS_TENSORSTORE is set to OFF.
#   2. TensorStore_FOUND is FALSE.
#   3. A "CMake Warning" appears in the captured output (R13.4 -
#      severity is WARNING when fallback is the *consequence* of
#      TensorStore being absent, not the user's explicit choice).
#   4. The fallback name "NCZarr fallback (sharding unavailable)"
#      appears (R13.4 - names the fallback mode).
#   5. The reason text "TensorStore not located via
#      find_package(TensorStore CONFIG)" appears (R13.4 - identifies
#      the reason TensorStore was not selected).
#
# We empty CMAKE_PREFIX_PATH and disable CMake's package registries
# in the sub-configure so any TensorStore install on the host cannot
# accidentally short-circuit the test.  This makes the test
# deterministic on developer machines that happen to have TensorStore
...
execute_process(
    COMMAND "${AMIO_TEST_CMAKE}"
            ...
            -DAMIO_FORCE_NCZARR=OFF
            -DAMIO_SOURCE_DIR=${AMIO_SOURCE_DIR}
            # Empty out every search path so TensorStore cannot be
...
if(_hit_reason EQUAL -1)
    list(APPEND _failures
         "fallback message did not identify the reason TensorStore was not selected")
endif()
