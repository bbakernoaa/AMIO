...
# `amio_discovery_force_nczarr_disables_tensorstore` CTest entry.
#
# Verifies (Mermaid path: A -> B[FORCE_NCZARR?] -- ON --> F):
#
#   1. AMIO_HAS_TENSORSTORE is set to OFF.
#   2. TensorStore_FOUND is FALSE (find_package was short-circuited
#      and not allowed to reach an upstream config file).
#   3. The reason text identifies AMIO_FORCE_NCZARR=ON as the cause.
#   4. FindTensorStore.cmake emits the documented STATUS line
#      "TensorStore discovery short-circuited (AMIO_FORCE_NCZARR=ON)".
#
# All four observables must be present.  This is AND semantics; we
# emit AMIO_DISCOVERY_FORCE_NCZARR=OK only when every check passes,
# AMIO_DISCOVERY_FORCE_NCZARR=FAIL otherwise (with a diagnostic listing
# which observable failed).
#
# Inputs (all -D flags from tests/cmake_discovery/CMakeLists.txt):
#   AMIO_TEST_CMAKE      - path to the cmake executable to invoke
#   AMIO_TEST_GENERATOR  - CMake generator string for the sub-build
#   AMIO_TEST_PROBE_SRC  - path to the probes/force_nczarr/ source
#   AMIO_TEST_PROBE_BIN  - path to a writable build directory
#   AMIO_SOURCE_DIR      - root of the AMIO repository (so the probe
#                          can pull cmake/FindTensorStore.cmake from
#                          the source tree)
####################################################################
...
# Wipe any prior build tree so cache state from a previous run cannot
# leak into the test.
...
execute_process(
    COMMAND "${AMIO_TEST_CMAKE}"
            -S "${AMIO_TEST_PROBE_SRC}"
            -B "${AMIO_TEST_PROBE_BIN}"
...
string(FIND "${_combined}"
       "TensorStore discovery short-circuited (AMIO_FORCE_NCZARR=ON)"
       _hit_status)
if(_hit_status EQUAL -1)
    list(APPEND _failures
         "FindTensorStore.cmake did not emit the documented short-circuit STATUS")
endif()
