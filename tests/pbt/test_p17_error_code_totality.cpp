...
TEST_CASE("P17: Error code totality - strerror always returns non-null",
          "[pbt][p17][error_code][strerror][totality]") {
    auto result = rc::check(
        "amio_strerror(i) returns non-null null-terminated string for any int",
        []() {
...
TEST_CASE("P17: Error code totality - defined codes byte-equal across calls",
          "[pbt][p17][error_code][strerror][idempotent]") {
    auto result = rc::check(
        "defined codes return byte-equal descriptions across invocations",
        []() {
            // Pick a defined code.
...
TEST_CASE("P17: Error code totality - undefined codes return unknown description",
          "[pbt][p17][error_code][strerror][undefined]") {
    auto result = rc::check(
        "undefined codes return AMIO_ERR_UNKNOWN(<int>) description",
        []() {
            // Generate an int that is NOT a defined code.
            // Strategy: generate from ranges that don't overlap with [0, 17].
            auto code = *rc::gen::suchThat<int>(
                rc::gen::inRange(-500, 500),
                [](int c) { return !is_defined_code(c); });
...
TEST_CASE("P17: Error code totality - undefined codes do not mutate table",
          "[pbt][p17][error_code][strerror][no_mutation]") {
    auto result = rc::check(
        "undefined code lookups do not mutate the defined-code table",
        []() {
...
TEST_CASE("P17: Error code totality - API returns only defined codes",
          "[pbt][p17][error_code][api_returns]") {
    auto result = rc::check(
        "API calls return only AMIO_OK or defined AMIO_ERR_* codes",
        []() {
            ...
            // amio_write with null dataset.
            {
                ...
                amio_status_t rc_val = amio_write(nullptr, "var", nullptr,
                                                   AMIO_DTYPE_F32, nullptr, &io);
                RC_ASSERT(is_defined_code(rc_val));
            }
