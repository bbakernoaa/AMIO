...
std::string write_dataset_config(const amio::pbt::TempDir& dir,
...
TEST_CASE("P1: Round_Trip_Equivalence - netcdf4 backend",
          "[pbt][p1][round_trip][netcdf4]") {
    auto result = rc::check(
        "write -> mutate host -> flush -> read yields bit-for-bit identical data (netcdf4)",
        []() {
            ...
            std::string dataset_config_path = write_dataset_config(
                tmp, backend, output_file, "dataset_write.yaml");
            ...
            amio_status_t rc_open = amio_open_dataset(
                guard.handle(), dataset_config_path.c_str(),
                AMIO_MODE_WRITE, &write_ds);
            ...
            amio_status_t rc_write = amio_write(
                write_ds,
                "test_var",
                host_buffer.data(),
                dtype,
            ...
            std::string dataset_read_config_path = write_dataset_config(
                tmp, backend, output_file, "dataset_read.yaml");

            amio_dataset_handle read_ds = nullptr;
            amio_status_t rc_open_r = amio_open_dataset(
                guard.handle(), dataset_read_config_path.c_str(),
                AMIO_MODE_READ, &read_ds);
            RC_ASSERT(rc_open_r == AMIO_OK);
            RC_ASSERT(read_ds != nullptr);

            // ---- Read back the variable ----
            amio_view_handle view = nullptr;
            amio_status_t rc_read = amio_read(
                read_ds,
                "test_var",
                0,       // timestep 0
...
TEST_CASE("P1: Round_Trip_Equivalence - zarr3 backend",
          "[pbt][p1][round_trip][zarr3]") {
    auto result = rc::check(
        "write -> mutate host -> flush -> read yields bit-for-bit identical data (zarr3)",
        []() {
            // ---- Generate random dtype and bounded shape ----
            auto dtype = *rc::gen::arbitrary<amio_dtype_t>();
            auto shape = *genBoundedShape(dtype);
            ...
            // Write dataset config YAML for amio_open_dataset.
            std::string dataset_config_path = write_dataset_config(
                tmp, backend, output_dir, "dataset_write.yaml");
            ...
            // ---- Open dataset for reading ----
            std::string dataset_read_config_path = write_dataset_config(
                tmp, backend, output_dir, "dataset_read.yaml");
