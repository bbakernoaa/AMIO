...
std::vector<std::string> valid_drt_names() {
    return {
        "adaptive_entropy_coding",
        "libaec",
        "adaptive entropy coding via libaec",
...
TEST_CASE("P25: GRIB2 metadata invariant - valid DRT names accepted",
          "[pbt][p25][grib2][metadata][drt_valid]") {
    auto result = rc::check(
        "valid DRT names are parsed without exception",
        []() {
            ...
            RC_ASSERT(drt == GRIB2_DRT::AdaptiveEntropyCoding ||
                      drt == GRIB2_DRT::LosslessJPEG2000);
        });
...
TEST_CASE("P25: GRIB2 metadata invariant - invalid DRT names rejected",
          "[pbt][p25][grib2][metadata][drt_invalid]") {
    auto result = rc::check(
        "invalid DRT names throw eckit::Exception",
        []() {
...
TEST_CASE("P25: GRIB2 metadata invariant - missing WMO key throws",
          "[pbt][p25][grib2][metadata][wmo_missing]") {
    auto result = rc::check(
        "missing WMO metadata key throws eckit::Exception",
        []() {
...
TEST_CASE("P25: GRIB2 metadata invariant - valid WMO keys translate",
          "[pbt][p25][grib2][metadata][wmo_valid]") {
    auto result = rc::check(
        "valid WMO metadata keys translate to integer codes",
        []() {
...
TEST_CASE("P25: GRIB2 metadata invariant - DRT missing vs unrecognized",
          "[pbt][p25][grib2][metadata][drt_distinction]") {
    auto result = rc::check(
        "DRT validation distinguishes missing from unrecognized",
        []() {
            ...
            RC_ASSERT(error_msg.find("unrecognized") != std::string::npos ||
