    ...
    return rc::gen::exec([]() {
        ...
        switch (category) {
            case 0:
                ...
                return std::string("s3://nonexistent-bucket-xyz-99999/"
                                   "unreachable/path/data.zarr");
            case 1:
                // Unreachable GCS URI (network failure).
                return std::string("gs://nonexistent-bucket-xyz-99999/"
                                   "unreachable/path/data.zarr");
            case 2:
                // Invalid HTTPS URI (network failure).
                return std::string("https://unreachable.invalid.host.example/"
            ...
            default:
                ...
                return std::string("/nonexistent/root/path/that/does/"
                                   "not/exist/data.zarr");
        }
    });
}
...
std::string expected_error_category(const std::string& uri) {
    ...
    if (uri.find("s3://") == 0 || uri.find("gs://") == 0 ||
        uri.find("https://") == 0) {
...
TEST_CASE("P26: Zarr cloud failure idempotence - URI classification",
          "[pbt][p26][zarr][cloud][uri_classification]") {
    auto result = rc::check(
        "cloud URIs are correctly classified",
        []() {
            ...
            bool expected_cloud = (uri.find("s3://") == 0 ||
                                   uri.find("gs://") == 0 ||
                                   uri.find("https://") == 0);
...
TEST_CASE("P26: Zarr cloud failure idempotence - error categorization",
          "[pbt][p26][zarr][cloud][error_categorization]") {
    auto result = rc::check(
        "error categorization produces non-empty labels",
        []() {
...
TEST_CASE("P26: Zarr cloud failure idempotence - no artifacts on failure",
          "[pbt][p26][zarr][cloud][no_artifacts]") {
    auto result = rc::check(
        "failed writes leave no artifacts at target path",
        []() {
            ...
            TempDir tmp;
            std::string nonexistent_subdir = tmp.file(
                "nonexistent_deep/nested/path/data.zarr");
...
TEST_CASE("P26: Zarr cloud failure idempotence - existing content unchanged",
          "[pbt][p26][zarr][cloud][content_unchanged]") {
    auto result = rc::check(
        "existing file content unchanged after failed write",
        []() {
            TempDir tmp;
            ...
            for (std::size_t i = 0; i < content_size; ++i) {
                original_content[i] = static_cast<uint8_t>(
            ...
            // Write the original content to the file.
            {
                ...
                ofs.write(reinterpret_cast<const char*>(original_content.data()),
            ...
            // failure scenario.
            {
                ...
                ifs.read(reinterpret_cast<char*>(read_back.data()),
                         static_cast<std::streamsize>(content_size));
...
TEST_CASE("P26: Zarr cloud failure idempotence - unsupported schemes",
          "[pbt][p26][zarr][cloud][unsupported_scheme]") {
    auto result = rc::check(
        "unsupported URI schemes are detected and categorized",
        []() {
            // Generate URIs with unsupported schemes.
            auto unsupported_schemes = std::vector<std::string>{
                "ftp://", "hdfs://", "nfs://", "smb://",
                "file://", "ssh://", "rsync://", "webdav://"
            };

            auto idx = *rc::gen::inRange<std::size_t>(
                0, unsupported_schemes.size());
