...
TEST_CASE("P12: Prefetch look-ahead replenishment - T+N within bounds",
          "[pbt][p12][prefetch][replenishment]") {
    auto result = rc::check(
        "after reading T, if T+N < M then fetch for T+N is scheduled",
        []() {
            ...
            auto total_timesteps = *rc::gen::inRange<std::int64_t>(
                static_cast<std::int64_t>(depth) + 1,
                static_cast<std::int64_t>(depth) + 65);

            // Timestep T in [0, min(N,M)-1] -- initially prefetched.
            std::int64_t max_prefetched = std::min(
...
TEST_CASE("P12: Prefetch look-ahead replenishment - T+N out of bounds",
          "[pbt][p12][prefetch][replenishment][out_of_bounds]") {
    auto result = rc::check(
        "after reading T, if T+N >= M then no additional fetch is scheduled",
        []() {
            // Generate look-ahead depth N in [1, 32].
            auto depth = *rc::gen::inRange<std::size_t>(1, 33);
            ...
            auto total_timesteps = *rc::gen::inRange<std::int64_t>(1,
                static_cast<std::int64_t>(depth) + 1);
...
TEST_CASE("P12: Prefetch look-ahead replenishment - sliding window",
          "[pbt][p12][prefetch][replenishment][sliding]") {
    auto result = rc::check(
        "sequential reads with schedule_next maintain sliding look-ahead window",
        []() {
            ...
            auto total_timesteps = *rc::gen::inRange<std::int64_t>(
                static_cast<std::int64_t>(2 * depth),
                static_cast<std::int64_t>(2 * depth) + 32);
            ...
            std::int64_t reads_to_do = std::min(
                static_cast<std::int64_t>(depth), total_timesteps);
