    ...
    void read(StagingBuffer& dst,
        ...
        if (fail_timesteps_.count(timestep) > 0) {
            throw std::runtime_error(
                "Injected read failure for timestep " +
...
TEST_CASE("P14: Prefetch failure surfaces - failed fetch returns error on read",
          "[pbt][p14][prefetch][failure]") {
    auto result = rc::check(
        "failed background fetch surfaces as AMIO_ERR_* on next amio_read(T)",
        []() {
...
TEST_CASE("P14: Prefetch failure surfaces - multiple failures retained",
          "[pbt][p14][prefetch][failure][multiple]") {
    auto result = rc::check(
        "multiple failed fetches are retained and surfaced independently",
        []() {
            ...
            auto total_timesteps = *rc::gen::inRange<std::int64_t>(
                ...
                static_cast<std::int64_t>(depth) + 32);
            ...
            num_failures = std::min(num_failures,
                                    static_cast<std::size_t>(max_prefetched));
...
TEST_CASE("P14: Prefetch failure surfaces - async with Worker_Pool",
          "[pbt][p14][prefetch][failure][async]") {
    auto result = rc::check(
        "async failed fetch surfaces as AMIO_ERR_* after Worker_Pool drain",
        []() {
