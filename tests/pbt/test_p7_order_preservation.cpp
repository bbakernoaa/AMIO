...
TEST_CASE("P7: Order preservation - same (dataset, variable) pair",
          "[pbt][p7][order_preservation]") {
    auto result = rc::check(
        "writes to same (dataset, variable) are observed in submission order",
        []() {
            ...
            for (std::size_t i = 0; i < num_writes; ++i) {
                auto seq = pool.submit_write(dv_key, [recorder, dv_key, i]() {
                    recorder->record(dv_key, i);
                });
                RC_ASSERT(seq == i);
            }
...
TEST_CASE("P7: Order preservation - MockBackendDriver records in order",
          "[pbt][p7][order_preservation][mock_driver]") {
    auto result = rc::check(
        "MockBackendDriver observes writes in submission order per (dataset, variable)",
        []() {
...
TEST_CASE("P7: Different (dataset, variable) pairs can interleave freely",
          "[pbt][p7][order_preservation][interleave]") {
    auto result = rc::check(
        "writes to different (dataset, variable) pairs are all completed and per-pair order is preserved",
        []() {
            ...
            for (std::size_t w = 0; w < writes_per_pair; ++w) {
                for (std::size_t p = 0; p < num_pairs; ++p) {
                    auto key = keys[p];
                    pool.submit_write(key, [recorder, key, w]() {
                        recorder->record(key, w);
                    });
                }
            }
