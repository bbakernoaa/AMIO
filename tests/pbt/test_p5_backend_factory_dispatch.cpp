...
rc::Gen<std::string> genAlphaString() {
    return rc::gen::exec([]() {
        ...
        for (std::size_t i = 0; i < len; ++i) {
            ...
            switch (kind) {
                case 0: s += static_cast<char>(*rc::gen::inRange(static_cast<int>('a'), static_cast<int>('z') + 1)); break;
                case 1: s += static_cast<char>(*rc::gen::inRange(static_cast<int>('A'), static_cast<int>('Z') + 1)); break;
                case 2: s += static_cast<char>(*rc::gen::inRange(static_cast<int>('0'), static_cast<int>('9') + 1)); break;
            }
        }
        return s;
    });
    ...
    return rc::gen::exec([registered_keys]() -> std::string {
        ...
        switch (strategy) {
            ...
            case 3: {
                ...
                while (std::find(registered_keys.begin(), registered_keys.end(), s)
                       != registered_keys.end()) {
                    s += "x";
                }
                return s;
...
public:
    explicit FactoryTestGuard(const std::vector<std::string>& keys) {
        ...
        for (const auto& key : keys) {
            factory.register_driver(key, []() -> std::unique_ptr<amio::detail::Backend_Driver> {
                return std::make_unique<amio::pbt::MockBackendDriver>();
            });
        }
    }
...
TEST_CASE("P5: exact-match returns correct driver for registered keys",
          "[pbt][factory]") {
...
TEST_CASE("P5: non-match returns AMIO_ERR_UNKNOWN_BACKEND, zero state mutation",
          "[pbt][factory]") {
...
TEST_CASE("P5: wrong case is a non-match (case-sensitive)",
          "[pbt][factory]") {
              ...
              []() {
                  ...
                  if (!flipped.empty()) {
                      ...
                      if (std::islower(static_cast<unsigned char>(c))) {
                          flipped[char_idx] = static_cast<char>(
...
TEST_CASE("P5: empty string returns AMIO_ERR_UNKNOWN_BACKEND",
          "[pbt][factory]") {
...
TEST_CASE("P5: concurrent read+write datasets resolve independently",
          "[pbt][factory]") {
