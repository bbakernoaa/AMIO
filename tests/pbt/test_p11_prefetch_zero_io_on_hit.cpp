...
TEST_CASE("P11: Prefetch zero-I/O on hit - no driver read on completed buffer",
          "[pbt][p11][prefetch][zero_io]") {
    auto result = rc::check(
        "amio_read(T) with completed buffer does not invoke Backend_Driver::read",
        []() {
...
TEST_CASE("P11: Prefetch zero-I/O on hit - sequential reads all from cache",
          "[pbt][p11][prefetch][zero_io][sequential]") {
    auto result = rc::check(
        "sequential reads of prefetched timesteps produce zero additional I/O",
        []() {
            ...
            std::int64_t prefetched_count = std::min(
                static_cast<std::int64_t>(depth), total_timesteps);
