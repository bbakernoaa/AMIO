...
TEST_CASE("PBT infrastructure: Arbitrary<amio_dtype_t> generates valid dtypes",
          "[pbt][infrastructure][dtype]") {
    rc::check("all generated dtypes are in [0, 9]",
              [](amio_dtype_t dtype) {
...
TEST_CASE("PBT infrastructure: Arbitrary<amio_shape_t> generates valid shapes",
          "[pbt][infrastructure][shape]") {
    rc::check("all generated shapes have valid rank and extents",
              [](amio_shape_t shape) {
                  // Rank in [1, 7].
...
TEST_CASE("PBT infrastructure: Arbitrary<Payload> generates valid payloads",
          "[pbt][infrastructure][payload]") {
    rc::check("payload byte count matches shape * dtype_size",
              [](amio::pbt::Payload payload) {
                  std::size_t expected = amio::pbt::payload_byte_count(
...
TEST_CASE("PBT infrastructure: Arbitrary<CodecConfig> generates valid configs",
          "[pbt][infrastructure][codec]") {
    rc::check("codec config has active codec on allow-list",
              [](amio::detail::CodecConfig cfg) {
                  ...
                  auto it = std::find(cfg.lossless_allow_list.begin(),
                                      cfg.lossless_allow_list.end(),
                                      cfg.active_codec);
...
TEST_CASE("PBT infrastructure: Arbitrary<Config> generates valid manifests",
          "[pbt][infrastructure][manifest]") {
    using namespace amio::detail;

    rc::check("all generated configs have fields in valid ranges",
              [](Config cfg) {
                  // Staging pool ranges.
...
TEST_CASE("PBT infrastructure: TempDir creates and manages temp directories",
          "[pbt][infrastructure][tempdir]") {
...
TEST_CASE("PBT infrastructure: manifest YAML helpers produce parseable files",
          "[pbt][infrastructure][manifest_yaml]") {
    amio::pbt::TempDir tmp;
...
TEST_CASE("PBT infrastructure: amio_strerror is accessible",
          "[pbt][infrastructure][api]") {
